...
class Address extends EasypostResource
{
    ...
    {		
        ...
        $requestor = new \EasyPost\Requestor($apiKey);		
        $url = self::classUrl($class);		
		
        list($response, $apiKey) = $requestor->request('post', $url.'/create_and_verify', $params);		
		
        if (isset($response['address'])) {
            $verified_address = \EasyPost\Util::convertToEasyPostObject($response['address'], $apiKey);
            ...
            return \EasyPost\Util::convertToEasyPostObject($response, $apiKey);
        }
    }
