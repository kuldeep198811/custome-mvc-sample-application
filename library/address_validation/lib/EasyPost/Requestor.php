...
class Requestor
{
    ...
    private function _requestRaw($method, $url, $params, $apiKeyRequired)
    {
        ...
        if (!$myApiKey) {
            if (!$myApiKey = \EasyPost\EasyPost::$apiKey) {
                if ($apiKeyRequired) {
                    throw new \EasyPost\Error('No API key provided. Set your API key using "EasyPost::setApiKey(<API-KEY>)". See https://www.easypost.com/docs for details, or contact support@easypost.com for assistance.');
                }
            }
        }
        ...
        $uname = php_uname();
        $ua = array(
            'bindings_version' => \EasyPost\EasyPost::VERSION,
            'lang'             => 'php',
        ...
        $headers = array('X-EasyPost-Client-User-Agent: ' . json_encode($ua),
            'User-Agent: EasyPost/v2 PhpClient/' . \EasyPost\EasyPost::VERSION,
            "Authorization: Bearer {$myApiKey}");
        if (\EasyPost\EasyPost::$apiVersion) {
            $headers[] = 'EasyPost-Version: ' . \EasyPost\EasyPost::$apiVersion;
        }		
    ...
    private function _interpretResponse($httpBody, $httpStatus)
    {
        try {
            ...
            throw new \EasyPost\Error("Invalid response body from API: HTTP Status: ({$httpStatus}) {$httpBody}", $httpStatus, $httpBody);
        }			
    ...
    public function handleApiError($httpBody, $httpStatus, $response)
    {
        ...
        if (!is_array($response) || !isset($response['error'])) {
            throw new \EasyPost\Error("Invalid response object from API: HTTP Status: ({$httpStatus}) {$httpBody})", $httpStatus, $httpBody);
        }
        throw new \EasyPost\Error(is_array($response['error']) ? $response['error']['message'] : (!empty($response['error']) ? $response['error'] : ""), $httpStatus, $httpBody);
    }
